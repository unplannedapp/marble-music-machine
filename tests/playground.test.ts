...
function runLevel(level: LevelDef, seconds = 40): RunResult {
...
describe('playground level', () => {
  it('marble travels the whole machine through real contacts without tunnelling', async () => {
    ...
    // Reached the bottom via the objects, in machine order.
    expect(r.order).toContain('rail_start');
    for (let i = 1; i <= 7; i++) expect(r.order).toContain(`pad_${i}`);
    expect(r.order.indexOf('pad_7')).toBeGreaterThan(r.order.indexOf('pad_1'));
